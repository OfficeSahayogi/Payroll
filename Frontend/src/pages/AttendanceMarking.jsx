...
const AttendanceMarking = () => {
  ...
  const handleDoubleClick = (employeeId, status) => {
    console.log("employeeId",employeeId,"status",status)
  ...
  const handleFieldChange = (employeeId, status, value) => {
    console.log("em",employeeId,"st",status,"vale",value)
  ...
  const handleManualHalfDaySubmit = () => {
    const codes = halfDayEmployeeCodes.split(",").map((code) => code.trim());
    const updatedData = filemployees.map((emp) => {
      if (codes.includes(emp.empCode)) {
      ...
      <div className="w-full max-w-6xl 2xl:max-w-full bg-white rounded-lg shadow-md p-2">
        <div className="flex justify-between mb-2">
          ...
          <button
              ...
              className="bg-yellow-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-yellow-500"
            >
              Mark Half-Day
            </button>
        </div>
        ...
        {!selectedDate ? (
          ...
          <>
            <div className="overflow-x-auto">
              <div className="max-h-[300px] 2xl:max-h-[600px] overflow-y-auto">
                    ...
                    <tr>
                      ...
                      <th className="px-4 py-2 border-b flex flex-col">
                        ...
                        <div className="flex justify-center gap-4 w-full">
                          <div className="flex items-center justify-evenly bg-yellow-100 w-full">
                  ...
                  <tbody className="text-center">
                    {filemployees.map((employee) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 border-b">
                          {employee.dol===null?"---":new Date(employee.dol)
                            .toLocaleDateString("en-GB")
                            .replace(/\//g, "-")}
                        </td>
                        <td className="px-4 py-2 border-b">
                          ...
                          {["Present", "Absent", "Half-Day"].map((status) => (
                            <div key={status} >
                              <input
                                ...
                                onDoubleClick={() => handleDoubleClick(employee.employeeId
                                  , status)}
                                onChange={(e) =>
                                  handleFieldChange(employee.employeeId
                                    , status, Number(e.target.value))
                                }
                                readOnly={editMode[employee.employeeId
                                ] !== status}
                        ...
                        <div className="flex gap-1 justify-evenly w-full">
                          ...
                          <input
                            type="text"
                            value={employee.status === "Half-Day" ? "H" : ""}
                            readOnly
                            className="border border-gray-300 rounded-lg p-1 w-20 bg-slate-200"
                          />
                        </div>
      ...
      {showManualAbsentModal && selectedDate && (
          <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-lg shadow-md w-1/3">
              ...
              <div className="mb-4">
                <label className="block text-sm font-semibold">Employee Codes (comma-separated):</label>
                <input
                  type="text"
                ...
                />
                <p className="text-sm text-gray-500 mt-2">Hover over the input for more info.</p>
              </div>
              <button
